...
export default async function InspectionData() {
  ...
  const totalInspections = status.passed + status.failed + status.requiresWork;
  

  const passedPercentage = Math.round((status.passed / totalInspections) * 100);
  const failedPercentage = Math.round((status.failed / totalInspections) * 100);
  const requiresWorkPercentage = Math.round((status.requiresWork / totalInspections) * 100);

  return (
    <div className="flex flex-col w-80 h-auto gap-4 p-4 bg-white rounded-lg shadow-sm">
     
      <div className="text-center mb-2">
        <h2 className="text-lg font-bold text-gray-800">Inspection Overview</h2>
        <p className="text-sm text-gray-600">Total Inspections: {totalInspections}</p>
      </div>
